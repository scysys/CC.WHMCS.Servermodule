...
$urlSHOUTcastSP = str_replace('/stream', '', $url);
//$urlIcecastSP = str_replace("/stream", "", $url);
...
// Type of streaming server used, shoutcast, icecastkh or icecastv2 supported
$streamingType = filter_input(INPUT_GET, 'streamtype', FILTER_SANITIZE_STRING);

// Get Mountpointnumber
$mountPointNumber = filter_input(INPUT_GET, 'mountpointnumber', FILTER_SANITIZE_STRING);

if (!empty($urlSHOUTcastSP)) {
    if ('shoutcast' === $streamingType) {
        if ($historic) {
            $urls[] = $urlSHOUTcastSP.'/7.html';
            $urls[] = $urlSHOUTcastSP.'/played';
            if ($nextSong) {
                $urls[] = $urlSHOUTcastSP.'/nextsong';
            }
            ...
            $curl = curl_init($urlSHOUTcastSP.'/7.html');
        ...
        if (!empty($data)) {
            ...
            if (2 === count($currentSong)) {
                if ('</body></html>' === substr($currentSong[1], -14)) {
                ...
                $array['currentSong'] = (empty($playingNow) || '</body></html>' === $playingNow) ? '...' : trim(substr($playingNow, 0, -14));
                $array['currentArtist'] = '...';
            } else {
        ...
        // Icecast KH
    } elseif ('icecast' === $streamingType or 'icecastkh' === $streamingType) {
        ...
        if (!empty($data)) {
            ...
            if ('' == $ice_stats['icestats']['source']['yp_currently_playing']) {
                $ice_stats_source = $ice_stats['icestats']['source'][$mountPointNumber];
            } else {
            ...
            $track_history = file('../log/'.$_GET['userid'].'_02.log');
            $track_list = array_slice($track_history, 0, 20);
            if (false === stripos($track_history[0], $currently_playing[0].' - '.$currently_playing[1])) {
                array_unshift($track_list, $currently_playing[0].' - '.$currently_playing[1]."\n");
                file_put_contents('../log/'.$_GET['userid'].'_02.log', $track_list);
            }
        } else {
            $array = ['error' => 'Failed to fetch data'];
        }
    ...
    // Icecast V2
    } elseif ('icecastv2' === $streamingType) {
        $url_explode = explode('/', $url);
        array_pop($url_explode);
        $url = implode('/', $url_explode);
        $url = $url.'/status-json.xsl';
        ...
        if (!empty($data)) {
            ...
            if ('' == $ice_stats['icestats']['source']['title']) {
                $ice_stats_source = $ice_stats['icestats']['source'][$mountPointNumber];
            } else {
                $ice_stats_source = $ice_stats['icestats']['source'];
            }
